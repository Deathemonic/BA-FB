...
public struct CheatCodeListExcel : IFlatbufferObject
{
  ...
  public static Offset<Japan.CheatCodeListExcel> CreateCheatCodeListExcel(FlatBufferBuilder builder,
      long Id = 0,
  ...
  public static Offset<Japan.CheatCodeListExcel> EndCheatCodeListExcel(FlatBufferBuilder builder) {
    int o = builder.EndTable();
    return new Offset<Japan.CheatCodeListExcel>(o);
  }
  ...
  public void UnPackTo(CheatCodeListExcelT _o) {
    ...
    for (var _j = 0; _j < this.CheatCodeLength; ++_j) {_o.CheatCode.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.CheatCode(_j), key) : this.CheatCode(_j));}
    _o.InputTitle = new List<string>();
    for (var _j = 0; _j < this.InputTitleLength; ++_j) {_o.InputTitle.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.InputTitle(_j), key) : this.InputTitle(_j));}
    _o.Desc = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.Desc, key) : this.Desc;
  }
  public static Offset<Japan.CheatCodeListExcel> Pack(FlatBufferBuilder builder, CheatCodeListExcelT _o) {
    if (_o == null) return default(Offset<Japan.CheatCodeListExcel>);
		byte[] key = TableEncryptionService.CreateKey("CheatCodeList");
    ...
    if (_o.CheatCode != null) {
      ...
      for (var _j = 0; _j < __CheatCode.Length; ++_j) { __CheatCode[_j] = builder.CreateString((TableEncryptionService.UseEncryption ? TableEncryptionService.ConvertPack((string)_o.CheatCode[_j], key) : _o.CheatCode[_j])); }
    ...
    if (_o.InputTitle != null) {
      ...
      for (var _j = 0; _j < __InputTitle.Length; ++_j) { __InputTitle[_j] = builder.CreateString((TableEncryptionService.UseEncryption ? TableEncryptionService.ConvertPack((string)_o.InputTitle[_j], key) : _o.InputTitle[_j])); }
      _InputTitle = CreateInputTitleVector(builder, __InputTitle);
    }
    var _Desc = _o.Desc == null ? default(StringOffset) : builder.CreateString((TableEncryptionService.UseEncryption ? TableEncryptionService.ConvertPack((string)_o.Desc, key) : _o.Desc));
