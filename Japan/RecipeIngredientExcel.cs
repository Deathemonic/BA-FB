...
public struct RecipeIngredientExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(RecipeIngredientExcelT _o) {
    ...
    for (var _j = 0; _j < this.CostIdLength; ++_j) {_o.CostId.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.CostId(_j), key) : this.CostId(_j));}
    _o.CostAmount = new List<long>();
    for (var _j = 0; _j < this.CostAmountLength; ++_j) {_o.CostAmount.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.CostAmount(_j), key) : this.CostAmount(_j));}
    _o.IngredientParcelType = new List<Japan.ParcelType>();
    for (var _j = 0; _j < this.IngredientParcelTypeLength; ++_j) {_o.IngredientParcelType.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.IngredientParcelType(_j), key) : this.IngredientParcelType(_j));}
    _o.IngredientId = new List<long>();
    for (var _j = 0; _j < this.IngredientIdLength; ++_j) {_o.IngredientId.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.IngredientId(_j), key) : this.IngredientId(_j));}
    _o.IngredientAmount = new List<long>();
    for (var _j = 0; _j < this.IngredientAmountLength; ++_j) {_o.IngredientAmount.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.IngredientAmount(_j), key) : this.IngredientAmount(_j));}
    _o.CostTimeInSecond = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.CostTimeInSecond, key) : this.CostTimeInSecond;
  }
