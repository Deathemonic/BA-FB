...
public struct DefaultEchelonExcel : IFlatbufferObject
{
  ...
  public static Offset<Japan.DefaultEchelonExcel> CreateDefaultEchelonExcel(FlatBufferBuilder builder,
      int EchlonId = 0,
      long LeaderId = 0,
  ...
  public static Offset<Japan.DefaultEchelonExcel> EndDefaultEchelonExcel(FlatBufferBuilder builder) {
    int o = builder.EndTable();
    return new Offset<Japan.DefaultEchelonExcel>(o);
  }
  ...
  public void UnPackTo(DefaultEchelonExcelT _o) {
    ...
    _o.EchlonId = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((int)this.EchlonId, key) : this.EchlonId;
    _o.LeaderId = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)this.LeaderId, key) : this.LeaderId;
    _o.MainId = new List<long>();
    for (var _j = 0; _j < this.MainIdLength; ++_j) {_o.MainId.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)this.MainId(_j), key) : this.MainId(_j));}
    _o.SupportId = new List<long>();
    for (var _j = 0; _j < this.SupportIdLength; ++_j) {_o.SupportId.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)this.SupportId(_j), key) : this.SupportId(_j));}
    _o.TssId = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)this.TssId, key) : this.TssId;
  }
  public static Offset<Japan.DefaultEchelonExcel> Pack(FlatBufferBuilder builder, DefaultEchelonExcelT _o) {
    if (_o == null) return default(Offset<Japan.DefaultEchelonExcel>);
		byte[] key = TableEncryptionService.CreateKey("DefaultEchelon");
		var _EchlonId = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((int)_o.EchlonId, key) : _o.EchlonId;
		var _LeaderId = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)_o.LeaderId, key) : _o.LeaderId;
    var _mainId = default(VectorOffset);
    if (_o.MainId != null) {
      var __mainId = _o.MainId.Select(x => TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)x, key) : x).ToArray();
      _mainId = CreateMainIdVector(builder, __mainId);
    }
    var _supportId = default(VectorOffset);
    if (_o.SupportId != null) {
      var __supportId = _o.SupportId.Select(x => TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)x, key) : x).ToArray();
      _supportId = CreateSupportIdVector(builder, __supportId);
    }
		var _TssId = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)_o.TssId, key) : _o.TssId;
