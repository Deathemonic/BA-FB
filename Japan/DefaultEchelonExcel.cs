...
public struct DefaultEchelonExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(DefaultEchelonExcelT _o) {
    ...
    _o.EchlonId = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.EchlonId, key) : this.EchlonId;
    _o.LeaderId = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.LeaderId, key) : this.LeaderId;
    _o.MainId = new List<long>();
    for (var _j = 0; _j < this.MainIdLength; ++_j) {_o.MainId.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.MainId(_j), key) : this.MainId(_j));}
    _o.SupportId = new List<long>();
    for (var _j = 0; _j < this.SupportIdLength; ++_j) {_o.SupportId.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.SupportId(_j), key) : this.SupportId(_j));}
    _o.TssId = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.TssId, key) : this.TssId;
  }
