...
public struct CampaignChapterRewardExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(CampaignChapterRewardExcelT _o) {
    ...
    _o.CampaignChapterStar = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.CampaignChapterStar, key) : this.CampaignChapterStar;
    _o.ChapterRewardParcelType = new List<Japan.ParcelType>();
    for (var _j = 0; _j < this.ChapterRewardParcelTypeLength; ++_j) {_o.ChapterRewardParcelType.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.ChapterRewardParcelType(_j), key) : this.ChapterRewardParcelType(_j));}
    _o.ChapterRewardId = new List<long>();
    for (var _j = 0; _j < this.ChapterRewardIdLength; ++_j) {_o.ChapterRewardId.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.ChapterRewardId(_j), key) : this.ChapterRewardId(_j));}
    _o.ChapterRewardAmount = new List<int>();
    for (var _j = 0; _j < this.ChapterRewardAmountLength; ++_j) {_o.ChapterRewardAmount.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.ChapterRewardAmount(_j), key) : this.ChapterRewardAmount(_j));}
  }
