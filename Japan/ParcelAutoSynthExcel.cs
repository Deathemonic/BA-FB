...
public struct ParcelAutoSynthExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(ParcelAutoSynthExcelT _o) {
    ...
    _o.RequireParcelType = TableEncryptionService.UseEncryption ? (Japan.ParcelType)TableEncryptionService.Convert((int)this.RequireParcelType, key) : this.RequireParcelType;
    ...
    _o.ResultParcelType = TableEncryptionService.UseEncryption ? (Japan.ParcelType)TableEncryptionService.Convert((int)this.ResultParcelType, key) : this.ResultParcelType;
  ...
  public static Offset<Japan.ParcelAutoSynthExcel> Pack(FlatBufferBuilder builder, ParcelAutoSynthExcelT _o) {
    if (_o == null) return default(Offset<Japan.ParcelAutoSynthExcel>);
		byte[] key = TableEncryptionService.CreateKey("ParcelAutoSynth");
		var _RequireParcelType = TableEncryptionService.UseEncryption ? (Japan.ParcelType)TableEncryptionService.Convert((int)_o.RequireParcelType, key) : _o.RequireParcelType;
		...
		var _ResultParcelType = TableEncryptionService.UseEncryption ? (Japan.ParcelType)TableEncryptionService.Convert((int)_o.ResultParcelType, key) : _o.ResultParcelType;
