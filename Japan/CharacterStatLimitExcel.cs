// <auto-generated>
//  automatically generated by the FlatBuffers compiler, do not modify
// </auto-generated>

namespace Japan
{

using global::System;
using global::System.Collections.Generic;
using global::Kuroko.Crypto;
using global::Google.FlatBuffers;

public struct CharacterStatLimitExcel : IFlatbufferObject
{
  private Table __p;
  public ByteBuffer ByteBuffer { get { return __p.bb; } }
  public static void ValidateVersion() { FlatBufferConstants.FLATBUFFERS_25_2_10(); }
  public static CharacterStatLimitExcel GetRootAsCharacterStatLimitExcel(ByteBuffer _bb) { return GetRootAsCharacterStatLimitExcel(_bb, new CharacterStatLimitExcel()); }
  public static CharacterStatLimitExcel GetRootAsCharacterStatLimitExcel(ByteBuffer _bb, CharacterStatLimitExcel obj) { return (obj.__assign(_bb.GetInt(_bb.Position) + _bb.Position, _bb)); }
  public void __init(int _i, ByteBuffer _bb) { __p = new Table(_i, _bb); }
  public CharacterStatLimitExcel __assign(int _i, ByteBuffer _bb) { __init(_i, _bb); return this; }

  public long Id { get { int o = __p.__offset(4); return o != 0 ? __p.bb.GetLong(o + __p.bb_pos) : (long)0; } }
  public Japan.TacticEntityType TacticEntityType { get { int o = __p.__offset(6); return o != 0 ? (Japan.TacticEntityType)__p.bb.GetInt(o + __p.bb_pos) : Japan.TacticEntityType.None; } }
  public Japan.StatType StatType { get { int o = __p.__offset(8); return o != 0 ? (Japan.StatType)__p.bb.GetInt(o + __p.bb_pos) : Japan.StatType.None; } }
  public long StatMinValue { get { int o = __p.__offset(10); return o != 0 ? __p.bb.GetLong(o + __p.bb_pos) : (long)0; } }
  public long StatMaxValue { get { int o = __p.__offset(12); return o != 0 ? __p.bb.GetLong(o + __p.bb_pos) : (long)0; } }
  public long StatRatioMinValue { get { int o = __p.__offset(14); return o != 0 ? __p.bb.GetLong(o + __p.bb_pos) : (long)0; } }
  public long StatRatioMaxValue { get { int o = __p.__offset(16); return o != 0 ? __p.bb.GetLong(o + __p.bb_pos) : (long)0; } }

  public static Offset<Japan.CharacterStatLimitExcel> CreateCharacterStatLimitExcel(FlatBufferBuilder builder,
      long Id = 0,
      Japan.TacticEntityType TacticEntityType = Japan.TacticEntityType.None,
      Japan.StatType StatType = Japan.StatType.None,
      long StatMinValue = 0,
      long StatMaxValue = 0,
      long StatRatioMinValue = 0,
      long StatRatioMaxValue = 0) {
    builder.StartTable(7);
    CharacterStatLimitExcel.AddStatRatioMaxValue(builder, StatRatioMaxValue);
    CharacterStatLimitExcel.AddStatRatioMinValue(builder, StatRatioMinValue);
    CharacterStatLimitExcel.AddStatMaxValue(builder, StatMaxValue);
    CharacterStatLimitExcel.AddStatMinValue(builder, StatMinValue);
    CharacterStatLimitExcel.AddId(builder, Id);
    CharacterStatLimitExcel.AddStatType(builder, StatType);
    CharacterStatLimitExcel.AddTacticEntityType(builder, TacticEntityType);
    return CharacterStatLimitExcel.EndCharacterStatLimitExcel(builder);
  }

  public static void StartCharacterStatLimitExcel(FlatBufferBuilder builder) { builder.StartTable(7); }
  public static void AddId(FlatBufferBuilder builder, long id) { builder.AddLong(0, id, 0); }
  public static void AddTacticEntityType(FlatBufferBuilder builder, Japan.TacticEntityType tacticEntityType) { builder.AddInt(1, (int)tacticEntityType, 0); }
  public static void AddStatType(FlatBufferBuilder builder, Japan.StatType statType) { builder.AddInt(2, (int)statType, 0); }
  public static void AddStatMinValue(FlatBufferBuilder builder, long statMinValue) { builder.AddLong(3, statMinValue, 0); }
  public static void AddStatMaxValue(FlatBufferBuilder builder, long statMaxValue) { builder.AddLong(4, statMaxValue, 0); }
  public static void AddStatRatioMinValue(FlatBufferBuilder builder, long statRatioMinValue) { builder.AddLong(5, statRatioMinValue, 0); }
  public static void AddStatRatioMaxValue(FlatBufferBuilder builder, long statRatioMaxValue) { builder.AddLong(6, statRatioMaxValue, 0); }
  public static Offset<Japan.CharacterStatLimitExcel> EndCharacterStatLimitExcel(FlatBufferBuilder builder) {
    int o = builder.EndTable();
    return new Offset<Japan.CharacterStatLimitExcel>(o);
  }
  public CharacterStatLimitExcelT UnPack() {
    var _o = new CharacterStatLimitExcelT();
    this.UnPackTo(_o);
    return _o;
  }
  public void UnPackTo(CharacterStatLimitExcelT _o) {
		byte[] key = TableEncryptionService.CreateKey("CharacterStatLimit");
    _o.Id = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)this.Id, key) : this.Id;
    _o.TacticEntityType = TableEncryptionService.UseEncryption ? (Japan.TacticEntityType)TableEncryptionService.Convert((int)this.TacticEntityType, key) : this.TacticEntityType;
    _o.StatType = TableEncryptionService.UseEncryption ? (Japan.StatType)TableEncryptionService.Convert((int)this.StatType, key) : this.StatType;
    _o.StatMinValue = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)this.StatMinValue, key) : this.StatMinValue;
    _o.StatMaxValue = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)this.StatMaxValue, key) : this.StatMaxValue;
    _o.StatRatioMinValue = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)this.StatRatioMinValue, key) : this.StatRatioMinValue;
    _o.StatRatioMaxValue = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)this.StatRatioMaxValue, key) : this.StatRatioMaxValue;
  }
  public static Offset<Japan.CharacterStatLimitExcel> Pack(FlatBufferBuilder builder, CharacterStatLimitExcelT _o) {
    if (_o == null) return default(Offset<Japan.CharacterStatLimitExcel>);
		byte[] key = TableEncryptionService.CreateKey("CharacterStatLimit");
		var _Id = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)_o.Id, key) : _o.Id;
		var _TacticEntityType = TableEncryptionService.UseEncryption ? (Japan.TacticEntityType)TableEncryptionService.Convert((int)_o.TacticEntityType, key) : _o.TacticEntityType;
		var _StatType = TableEncryptionService.UseEncryption ? (Japan.StatType)TableEncryptionService.Convert((int)_o.StatType, key) : _o.StatType;
		var _StatMinValue = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)_o.StatMinValue, key) : _o.StatMinValue;
		var _StatMaxValue = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)_o.StatMaxValue, key) : _o.StatMaxValue;
		var _StatRatioMinValue = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)_o.StatRatioMinValue, key) : _o.StatRatioMinValue;
		var _StatRatioMaxValue = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)_o.StatRatioMaxValue, key) : _o.StatRatioMaxValue;
    return CreateCharacterStatLimitExcel(
      builder,
      TableEncryptionService.UseEncryption ? _Id : _o.Id,
      TableEncryptionService.UseEncryption ? _TacticEntityType : _o.TacticEntityType,
      TableEncryptionService.UseEncryption ? _StatType : _o.StatType,
      TableEncryptionService.UseEncryption ? _StatMinValue : _o.StatMinValue,
      TableEncryptionService.UseEncryption ? _StatMaxValue : _o.StatMaxValue,
      TableEncryptionService.UseEncryption ? _StatRatioMinValue : _o.StatRatioMinValue,
      TableEncryptionService.UseEncryption ? _StatRatioMaxValue : _o.StatRatioMaxValue);
  }
}

public class CharacterStatLimitExcelT
{
  public long Id { get; set; }
  public Japan.TacticEntityType TacticEntityType { get; set; }
  public Japan.StatType StatType { get; set; }
  public long StatMinValue { get; set; }
  public long StatMaxValue { get; set; }
  public long StatRatioMinValue { get; set; }
  public long StatRatioMaxValue { get; set; }

  public CharacterStatLimitExcelT() {
    this.Id = 0;
    this.TacticEntityType = Japan.TacticEntityType.None;
    this.StatType = Japan.StatType.None;
    this.StatMinValue = 0;
    this.StatMaxValue = 0;
    this.StatRatioMinValue = 0;
    this.StatRatioMaxValue = 0;
  }
}


static public class CharacterStatLimitExcelVerify
{
  static public bool Verify(Google.FlatBuffers.Verifier verifier, uint tablePos)
  {
    return verifier.VerifyTableStart(tablePos)
      && verifier.VerifyField(tablePos, 4 /*Id*/, 8 /*long*/, 8, false)
      && verifier.VerifyField(tablePos, 6 /*TacticEntityType*/, 4 /*Japan.TacticEntityType*/, 4, false)
      && verifier.VerifyField(tablePos, 8 /*StatType*/, 4 /*Japan.StatType*/, 4, false)
      && verifier.VerifyField(tablePos, 10 /*StatMinValue*/, 8 /*long*/, 8, false)
      && verifier.VerifyField(tablePos, 12 /*StatMaxValue*/, 8 /*long*/, 8, false)
      && verifier.VerifyField(tablePos, 14 /*StatRatioMinValue*/, 8 /*long*/, 8, false)
      && verifier.VerifyField(tablePos, 16 /*StatRatioMaxValue*/, 8 /*long*/, 8, false)
      && verifier.VerifyTableEnd(tablePos);
  }
}

}
