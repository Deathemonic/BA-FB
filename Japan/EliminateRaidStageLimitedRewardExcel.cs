...
public struct EliminateRaidStageLimitedRewardExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(EliminateRaidStageLimitedRewardExcelT _o) {
    ...
    _o.LimitedRewardId = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.LimitedRewardId, key) : this.LimitedRewardId;
    _o.LimitedRewardParcelType = new List<Japan.ParcelType>();
    for (var _j = 0; _j < this.LimitedRewardParcelTypeLength; ++_j) {_o.LimitedRewardParcelType.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.LimitedRewardParcelType(_j), key) : this.LimitedRewardParcelType(_j));}
    _o.LimitedRewardParcelUniqueId = new List<long>();
    for (var _j = 0; _j < this.LimitedRewardParcelUniqueIdLength; ++_j) {_o.LimitedRewardParcelUniqueId.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.LimitedRewardParcelUniqueId(_j), key) : this.LimitedRewardParcelUniqueId(_j));}
    _o.LimitedRewardAmount = new List<long>();
    for (var _j = 0; _j < this.LimitedRewardAmountLength; ++_j) {_o.LimitedRewardAmount.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.LimitedRewardAmount(_j), key) : this.LimitedRewardAmount(_j));}
  }
  public static Offset<Japan.EliminateRaidStageLimitedRewardExcel> Pack(FlatBufferBuilder builder, EliminateRaidStageLimitedRewardExcelT _o) {
    ...
    if (_o.LimitedRewardAmount != null) {
      var __LimitedRewardAmount = _o.LimitedRewardAmount.Select(x => (TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(x, key) : x)).ToArray();
