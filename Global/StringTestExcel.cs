...
public struct StringTestExcel : IFlatbufferObject
{
  ...
  public static Offset<Global.StringTestExcel> CreateStringTestExcel(FlatBufferBuilder builder,
  ...
  public static Offset<Global.StringTestExcel> EndStringTestExcel(FlatBufferBuilder builder) {
    int o = builder.EndTable();
    return new Offset<Global.StringTestExcel>(o);
  }
  ...
  public void UnPackTo(StringTestExcelT _o) {
    ...
    for (var _j = 0; _j < this.StringLength; ++_j) {_o.String.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.String(_j), key) : this.String(_j));}
    _o.Sentence1 = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.Sentence1, key) : this.Sentence1;
    _o.Script = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.Script, key) : this.Script;
  }
  public static Offset<Global.StringTestExcel> Pack(FlatBufferBuilder builder, StringTestExcelT _o) {
    if (_o == null) return default(Offset<Global.StringTestExcel>);
		byte[] key = TableEncryptionService.CreateKey("StringTest");
    var _String = default(VectorOffset);
    if (_o.String != null) {
      var __String = new StringOffset[_o.String.Count];
      for (var _j = 0; _j < __String.Length; ++_j) { __String[_j] = builder.CreateString((TableEncryptionService.UseEncryption ? TableEncryptionService.ConvertPack((string)_o.String[_j], key) : _o.String[_j])); }
      _String = CreateStringVector(builder, __String);
    }
    var _sentence1 = _o.Sentence1 == null ? default(StringOffset) : builder.CreateString((TableEncryptionService.UseEncryption ? TableEncryptionService.ConvertPack((string)_o.Sentence1, key) : _o.Sentence1));
    var _Script = _o.Script == null ? default(StringOffset) : builder.CreateString((TableEncryptionService.UseEncryption ? TableEncryptionService.ConvertPack((string)_o.Script, key) : _o.Script));
