...
public struct WorldRaidConditionExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(WorldRaidConditionExcelT _o) {
    ...
    for (var _j = 0; _j < this.ScenarioModeIdLength; ++_j) {_o.ScenarioModeId.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.ScenarioModeId(_j), key) : this.ScenarioModeId(_j));}
    _o.CampaignStageID = new List<long>();
    for (var _j = 0; _j < this.CampaignStageIDLength; ++_j) {_o.CampaignStageID.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.CampaignStageID(_j), key) : this.CampaignStageID(_j));}
    ...
    for (var _j = 0; _j < this.WorldRaidBossKillLength; ++_j) {_o.WorldRaidBossKill.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.WorldRaidBossKill(_j), key) : this.WorldRaidBossKill(_j));}
  }
  public static Offset<Global.WorldRaidConditionExcel> Pack(FlatBufferBuilder builder, WorldRaidConditionExcelT _o) {
    ...
    if (_o.ScenarioModeId != null) {
      var __ScenarioModeId = _o.ScenarioModeId.Select(x => (TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(x, key) : x)).ToArray();
    ...
    if (_o.CampaignStageID != null) {
      var __CampaignStageID = _o.CampaignStageID.Select(x => (TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(x, key) : x)).ToArray();
    ...
    var _AfterWhenDate = _o.AfterWhenDate == null ? default(StringOffset) : builder.CreateString((TableEncryptionService.UseEncryption ? TableEncryptionService.ConvertPack(_o.AfterWhenDate, key) : _o.AfterWhenDate));
    var _WorldRaidBossKill = default(VectorOffset);
    if (_o.WorldRaidBossKill != null) {
      var __WorldRaidBossKill = _o.WorldRaidBossKill.Select(x => (TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(x, key) : x)).ToArray();
