...
public struct FormationLocationExcel : IFlatbufferObject
{
  ...
  public static Offset<Global.FormationLocationExcel> CreateFormationLocationExcel(FlatBufferBuilder builder,
      long Id = 0,
      long GroupID = 0,
  ...
  public static Offset<Global.FormationLocationExcel> EndFormationLocationExcel(FlatBufferBuilder builder) {
    int o = builder.EndTable();
    return new Offset<Global.FormationLocationExcel>(o);
  }
  ...
  public void UnPackTo(FormationLocationExcelT _o) {
    ...
    for (var _j = 0; _j < this.SlotZLength; ++_j) {_o.SlotZ.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((float)this.SlotZ(_j), key) : this.SlotZ(_j));}
    _o.SlotX = new List<float>();
    for (var _j = 0; _j < this.SlotXLength; ++_j) {_o.SlotX.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((float)this.SlotX(_j), key) : this.SlotX(_j));}
  }
  public static Offset<Global.FormationLocationExcel> Pack(FlatBufferBuilder builder, FormationLocationExcelT _o) {
    if (_o == null) return default(Offset<Global.FormationLocationExcel>);
		byte[] key = TableEncryptionService.CreateKey("FormationLocation");
    ...
    if (_o.SlotZ != null) {
      var __SlotZ = _o.SlotZ.Select(x => TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((float)x, key) : x).ToArray();
      _SlotZ = CreateSlotZVector(builder, __SlotZ);
    }
    var _SlotX = default(VectorOffset);
    if (_o.SlotX != null) {
      var __SlotX = _o.SlotX.Select(x => TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((float)x, key) : x).ToArray();
