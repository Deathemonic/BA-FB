...
public struct CafeRankExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(CafeRankExcelT _o) {
    ...
    _o.ComfortMax = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.ComfortMax, key) : this.ComfortMax;
    _o.TagCountMax = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.TagCountMax, key) : this.TagCountMax;
    _o.CharacterVisitMin = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.CharacterVisitMin, key) : this.CharacterVisitMin;
    _o.CharacterVisitMax = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.CharacterVisitMax, key) : this.CharacterVisitMax;
    _o.CafeVisitWeightBase = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.CafeVisitWeightBase, key) : this.CafeVisitWeightBase;
    _o.CafeVisitWeightTagBonusStep = new List<int>();
    for (var _j = 0; _j < this.CafeVisitWeightTagBonusStepLength; ++_j) {_o.CafeVisitWeightTagBonusStep.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.CafeVisitWeightTagBonusStep(_j), key) : this.CafeVisitWeightTagBonusStep(_j));}
    _o.CafeVisitWeightTagBonus = new List<int>();
    for (var _j = 0; _j < this.CafeVisitWeightTagBonusLength; ++_j) {_o.CafeVisitWeightTagBonus.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.CafeVisitWeightTagBonus(_j), key) : this.CafeVisitWeightTagBonus(_j));}
  }
  public static Offset<Global.CafeRankExcel> Pack(FlatBufferBuilder builder, CafeRankExcelT _o) {
    if (_o == null) return default(Offset<Global.CafeRankExcel>);
		...
		var _ComfortMax = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(_o.ComfortMax, key) : _o.ComfortMax;
		var _TagCountMax = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(_o.TagCountMax, key) : _o.TagCountMax;
    ...
    if (_o.CafeVisitWeightTagBonus != null) {
      var __cafeVisitWeightTagBonus = _o.CafeVisitWeightTagBonus.Select(x => (TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(x, key) : x)).ToArray();
