...
public struct FavorLevelExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(FavorLevelExcelT _o) {
    ...
    for (var _j = 0; _j < this.ExpTypeLength; ++_j) {_o.ExpType.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.ExpType(_j), key) : this.ExpType(_j));}
  }
  public static Offset<Global.FavorLevelExcel> Pack(FlatBufferBuilder builder, FavorLevelExcelT _o) {
    ...
    if (_o.ExpType != null) {
      var __ExpType = _o.ExpType.Select(x => (TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(x, key) : x)).ToArray();
