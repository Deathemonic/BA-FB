...
public struct EventContentCharacterBonusExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(EventContentCharacterBonusExcelT _o) {
    ...
    for (var _j = 0; _j < this.EventContentItemTypeLength; ++_j) {_o.EventContentItemType.Add(TableEncryptionService.UseEncryption ? (Global.EventContentItemType)TableEncryptionService.Convert((int)this.EventContentItemType(_j), key) : this.EventContentItemType(_j));}
    _o.BonusPercentage = new List<long>();
    for (var _j = 0; _j < this.BonusPercentageLength; ++_j) {_o.BonusPercentage.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)this.BonusPercentage(_j), key) : this.BonusPercentage(_j));}
  }
  public static Offset<Global.EventContentCharacterBonusExcel> Pack(FlatBufferBuilder builder, EventContentCharacterBonusExcelT _o) {
    ...
    if (_o.EventContentItemType != null) {
      var __eventContentItemType = _o.EventContentItemType.Select(x => TableEncryptionService.UseEncryption ? (Global.EventContentItemType)TableEncryptionService.Convert((int)x, key) : x).ToArray();
    ...
    if (_o.BonusPercentage != null) {
      var __bonusPercentage = _o.BonusPercentage.Select(x => TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)x, key) : x).ToArray();
