...
public struct StatLevelInterpolationExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(StatLevelInterpolationExcelT _o) {
    ...
    for (var _j = 0; _j < this.StatTypeIndexLength; ++_j) {_o.StatTypeIndex.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.StatTypeIndex(_j), key) : this.StatTypeIndex(_j));}
  }
  public static Offset<Global.StatLevelInterpolationExcel> Pack(FlatBufferBuilder builder, StatLevelInterpolationExcelT _o) {
    ...
    if (_o.StatTypeIndex != null) {
      var __StatTypeIndex = _o.StatTypeIndex.Select(x => (TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(x, key) : x)).ToArray();
