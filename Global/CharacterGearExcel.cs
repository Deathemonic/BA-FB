...
public struct CharacterGearExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(CharacterGearExcelT _o) {
    ...
    _o.OpenFavorLevel = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.OpenFavorLevel, key) : this.OpenFavorLevel;
    ...
    for (var _j = 0; _j < this.MinStatValueLength; ++_j) {_o.MinStatValue.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.MinStatValue(_j), key) : this.MinStatValue(_j));}
    _o.MaxStatValue = new List<long>();
    for (var _j = 0; _j < this.MaxStatValueLength; ++_j) {_o.MaxStatValue.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.MaxStatValue(_j), key) : this.MaxStatValue(_j));}
  ...
  public static Offset<Global.CharacterGearExcel> Pack(FlatBufferBuilder builder, CharacterGearExcelT _o) {
    if (_o == null) return default(Offset<Global.CharacterGearExcel>);
		...
		var _OpenFavorLevel = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(_o.OpenFavorLevel, key) : _o.OpenFavorLevel;
		var _MaxLevel = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(_o.MaxLevel, key) : _o.MaxLevel;
    var _LearnSkillSlot = _o.LearnSkillSlot == null ? default(StringOffset) : builder.CreateString((TableEncryptionService.UseEncryption ? TableEncryptionService.ConvertPack(_o.LearnSkillSlot, key) : _o.LearnSkillSlot));
    ...
    if (_o.MinStatValue != null) {
      var __MinStatValue = _o.MinStatValue.Select(x => (TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(x, key) : x)).ToArray();
    ...
    if (_o.MaxStatValue != null) {
      var __MaxStatValue = _o.MaxStatValue.Select(x => (TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(x, key) : x)).ToArray();
