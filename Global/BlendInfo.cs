...
public struct BlendInfo : IFlatbufferObject
{
  ...
  public static Offset<Global.BlendInfo> CreateBlendInfo(FlatBufferBuilder builder,
      int From = 0,
      int To = 0,
      float blend = 0.0f) {
  ...
  public static Offset<Global.BlendInfo> EndBlendInfo(FlatBufferBuilder builder) {
    int o = builder.EndTable();
    return new Offset<Global.BlendInfo>(o);
  }
  ...
  public void UnPackTo(BlendInfoT _o) {
    ...
    _o.From = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((int)this.From, key) : this.From;
    _o.To = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((int)this.To, key) : this.To;
    _o.Blend = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((float)this.Blend, key) : this.Blend;
  }
  public static Offset<Global.BlendInfo> Pack(FlatBufferBuilder builder, BlendInfoT _o) {
    if (_o == null) return default(Offset<Global.BlendInfo>);
		byte[] key = TableEncryptionService.CreateKey("Blen");
		var _From = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((int)_o.From, key) : _o.From;
		var _To = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((int)_o.To, key) : _o.To;
		var _blend = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((float)_o.Blend, key) : _o.Blend;
    return CreateBlendInfo(
      builder,
      TableEncryptionService.UseEncryption ? _From : _o.From,
      TableEncryptionService.UseEncryption ? _To : _o.To,
      TableEncryptionService.UseEncryption ? _blend : _o.Blend);
  }
}
