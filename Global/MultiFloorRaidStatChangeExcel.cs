...
public struct MultiFloorRaidStatChangeExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(MultiFloorRaidStatChangeExcelT _o) {
    ...
    _o.StatChangeId = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.StatChangeId, key) : this.StatChangeId;
    ...
    for (var _j = 0; _j < this.StatAddLength; ++_j) {_o.StatAdd.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.StatAdd(_j), key) : this.StatAdd(_j));}
    _o.StatMultiply = new List<long>();
    for (var _j = 0; _j < this.StatMultiplyLength; ++_j) {_o.StatMultiply.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.StatMultiply(_j), key) : this.StatMultiply(_j));}
    _o.ApplyCharacterId = new List<long>();
    for (var _j = 0; _j < this.ApplyCharacterIdLength; ++_j) {_o.ApplyCharacterId.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.ApplyCharacterId(_j), key) : this.ApplyCharacterId(_j));}
  }
  public static Offset<Global.MultiFloorRaidStatChangeExcel> Pack(FlatBufferBuilder builder, MultiFloorRaidStatChangeExcelT _o) {
    if (_o == null) return default(Offset<Global.MultiFloorRaidStatChangeExcel>);
		byte[] key = TableEncryptionService.CreateKey("MultiFloorRaidStatChange");
		var _StatChangeId = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(_o.StatChangeId, key) : _o.StatChangeId;
    ...
    if (_o.StatAdd != null) {
      var __StatAdd = _o.StatAdd.Select(x => (TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(x, key) : x)).ToArray();
      _StatAdd = CreateStatAddVector(builder, __StatAdd);
    }
    var _StatMultiply = default(VectorOffset);
    if (_o.StatMultiply != null) {
      var __StatMultiply = _o.StatMultiply.Select(x => (TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(x, key) : x)).ToArray();
    ...
    if (_o.ApplyCharacterId != null) {
      var __ApplyCharacterId = _o.ApplyCharacterId.Select(x => (TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(x, key) : x)).ToArray();
