...
public struct MinigameTBGDiceExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(MinigameTBGDiceExcelT _o) {
    ...
    _o.DiceGroup = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.DiceGroup, key) : this.DiceGroup;
    _o.DiceResult = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.DiceResult, key) : this.DiceResult;
    ...
    for (var _j = 0; _j < this.ProbModifyConditionLength; ++_j) {_o.ProbModifyCondition.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.ProbModifyCondition(_j), key) : this.ProbModifyCondition(_j));}
    _o.ProbModifyValue = new List<int>();
    for (var _j = 0; _j < this.ProbModifyValueLength; ++_j) {_o.ProbModifyValue.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.ProbModifyValue(_j), key) : this.ProbModifyValue(_j));}
    _o.ProbModifyLimit = new List<int>();
    for (var _j = 0; _j < this.ProbModifyLimitLength; ++_j) {_o.ProbModifyLimit.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.ProbModifyLimit(_j), key) : this.ProbModifyLimit(_j));}
  }
