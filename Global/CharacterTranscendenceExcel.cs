...
public struct CharacterTranscendenceExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(CharacterTranscendenceExcelT _o) {
    ...
    for (var _j = 0; _j < this.MaxFavorLevelLength; ++_j) {_o.MaxFavorLevel.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.MaxFavorLevel(_j), key) : this.MaxFavorLevel(_j));}
    _o.StatBonusRateAttack = new List<long>();
    for (var _j = 0; _j < this.StatBonusRateAttackLength; ++_j) {_o.StatBonusRateAttack.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.StatBonusRateAttack(_j), key) : this.StatBonusRateAttack(_j));}
    _o.StatBonusRateHP = new List<long>();
    for (var _j = 0; _j < this.StatBonusRateHPLength; ++_j) {_o.StatBonusRateHP.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.StatBonusRateHP(_j), key) : this.StatBonusRateHP(_j));}
    _o.StatBonusRateHeal = new List<long>();
    for (var _j = 0; _j < this.StatBonusRateHealLength; ++_j) {_o.StatBonusRateHeal.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.StatBonusRateHeal(_j), key) : this.StatBonusRateHeal(_j));}
    ...
    for (var _j = 0; _j < this.MaxlevelStarLength; ++_j) {_o.MaxlevelStar.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.MaxlevelStar(_j), key) : this.MaxlevelStar(_j));}
  }
