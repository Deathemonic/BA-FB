...
public struct ScenarioEffectExcel : IFlatbufferObject
{
  ...
  public static Offset<Global.ScenarioEffectExcel> Pack(FlatBufferBuilder builder, ScenarioEffectExcelT _o) {
    if (_o == null) return default(Offset<Global.ScenarioEffectExcel>);
		byte[] key = TableEncryptionService.CreateKey("ScenarioEffect");
    var _EffectName = _o.EffectName == null ? default(StringOffset) : builder.CreateString((TableEncryptionService.UseEncryption ? TableEncryptionService.ConvertPack((string)_o.EffectName, key) : _o.EffectName));
