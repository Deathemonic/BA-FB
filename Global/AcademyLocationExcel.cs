...
public struct AcademyLocationExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(AcademyLocationExcelT _o) {
    ...
    for (var _j = 0; _j < this.OpenConditionLength; ++_j) {_o.OpenCondition.Add(TableEncryptionService.UseEncryption ? (Global.School)TableEncryptionService.Convert((int)this.OpenCondition(_j), key) : this.OpenCondition(_j));}
  ...
  public static Offset<Global.AcademyLocationExcel> Pack(FlatBufferBuilder builder, AcademyLocationExcelT _o) {
    ...
    if (_o.OpenCondition != null) {
      var __OpenCondition = _o.OpenCondition.Select(x => TableEncryptionService.UseEncryption ? (Global.School)TableEncryptionService.Convert((int)x, key) : x).ToArray();
