...
public struct AttendanceRewardExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(AttendanceRewardExcelT _o) {
    ...
    _o.AttendanceId = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)this.AttendanceId, key) : this.AttendanceId;
    _o.Day = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)this.Day, key) : this.Day;
  ...
  public static Offset<Global.AttendanceRewardExcel> Pack(FlatBufferBuilder builder, AttendanceRewardExcelT _o) {
    if (_o == null) return default(Offset<Global.AttendanceRewardExcel>);
		byte[] key = TableEncryptionService.CreateKey("AttendanceReward");
		var _AttendanceId = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)_o.AttendanceId, key) : _o.AttendanceId;
		var _Day = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((long)_o.Day, key) : _o.Day;
