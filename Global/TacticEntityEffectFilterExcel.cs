...
public struct TacticEntityEffectFilterExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(TacticEntityEffectFilterExcelT _o) {
    ...
    _o.ShowEffectToVehicle = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.ShowEffectToVehicle, key) : this.ShowEffectToVehicle;
    _o.ShowEffectToBoss = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.ShowEffectToBoss, key) : this.ShowEffectToBoss;
  }
  public static Offset<Global.TacticEntityEffectFilterExcel> Pack(FlatBufferBuilder builder, TacticEntityEffectFilterExcelT _o) {
    if (_o == null) return default(Offset<Global.TacticEntityEffectFilterExcel>);
		byte[] key = TableEncryptionService.CreateKey("TacticEntityEffectFilter");
    var _TargetEffectName = _o.TargetEffectName == null ? default(StringOffset) : builder.CreateString((TableEncryptionService.UseEncryption ? TableEncryptionService.ConvertPack(_o.TargetEffectName, key) : _o.TargetEffectName));
		var _ShowEffectToVehicle = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(_o.ShowEffectToVehicle, key) : _o.ShowEffectToVehicle;
		var _ShowEffectToBoss = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(_o.ShowEffectToBoss, key) : _o.ShowEffectToBoss;
