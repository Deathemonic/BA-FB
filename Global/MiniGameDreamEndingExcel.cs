...
public struct MiniGameDreamEndingExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(MiniGameDreamEndingExcelT _o) {
    ...
    for (var _j = 0; _j < this.EndingConditionLength; ++_j) {_o.EndingCondition.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.EndingCondition(_j), key) : this.EndingCondition(_j));}
    _o.EndingConditionValue = new List<long>();
    for (var _j = 0; _j < this.EndingConditionValueLength; ++_j) {_o.EndingConditionValue.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.EndingConditionValue(_j), key) : this.EndingConditionValue(_j));}
  }
  public static Offset<Global.MiniGameDreamEndingExcel> Pack(FlatBufferBuilder builder, MiniGameDreamEndingExcelT _o) {
    ...
    if (_o.EndingCondition != null) {
      var __EndingCondition = _o.EndingCondition.Select(x => (TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(x, key) : x)).ToArray();
    ...
    if (_o.EndingConditionValue != null) {
      var __EndingConditionValue = _o.EndingConditionValue.Select(x => (TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(x, key) : x)).ToArray();
