...
public struct PresetCharacterGroupSettingExcel : IFlatbufferObject
{
  ...
  public static Offset<Global.PresetCharacterGroupSettingExcel> CreatePresetCharacterGroupSettingExcel(FlatBufferBuilder builder,
      long CharacterId = 0,
  ...
  public static Offset<Global.PresetCharacterGroupSettingExcel> EndPresetCharacterGroupSettingExcel(FlatBufferBuilder builder) {
    int o = builder.EndTable();
    return new Offset<Global.PresetCharacterGroupSettingExcel>(o);
  }
  ...
  public void UnPackTo(PresetCharacterGroupSettingExcelT _o) {
    ...
    _o.ArenaSimulatorFixed = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((bool)this.ArenaSimulatorFixed, key) : this.ArenaSimulatorFixed;
    _o.PresetType = new List<string>();
    for (var _j = 0; _j < this.PresetTypeLength; ++_j) {_o.PresetType.Add(TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.PresetType(_j), key) : this.PresetType(_j));}
  }
  public static Offset<Global.PresetCharacterGroupSettingExcel> Pack(FlatBufferBuilder builder, PresetCharacterGroupSettingExcelT _o) {
    if (_o == null) return default(Offset<Global.PresetCharacterGroupSettingExcel>);
		byte[] key = TableEncryptionService.CreateKey("PresetCharacterGroupSetting");
		...
		var _ArenaSimulatorFixed = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert((bool)_o.ArenaSimulatorFixed, key) : _o.ArenaSimulatorFixed;
    var _PresetType = default(VectorOffset);
    if (_o.PresetType != null) {
      var __PresetType = new StringOffset[_o.PresetType.Count];
      for (var _j = 0; _j < __PresetType.Length; ++_j) { __PresetType[_j] = builder.CreateString((TableEncryptionService.UseEncryption ? TableEncryptionService.ConvertPack((string)_o.PresetType[_j], key) : _o.PresetType[_j])); }
    ...
    return CreatePresetCharacterGroupSettingExcel(
      ...
      TableEncryptionService.UseEncryption ? _ArenaSimulatorFixed : _o.ArenaSimulatorFixed,
      _PresetType);
  }
}
