...
public struct PresetCharacterGroupSettingExcel : IFlatbufferObject
{
  ...
  public void UnPackTo(PresetCharacterGroupSettingExcelT _o) {
    ...
    _o.ArenaSimulatorFixed = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(this.ArenaSimulatorFixed, key) : this.ArenaSimulatorFixed;
  ...
  public static Offset<Global.PresetCharacterGroupSettingExcel> Pack(FlatBufferBuilder builder, PresetCharacterGroupSettingExcelT _o) {
    if (_o == null) return default(Offset<Global.PresetCharacterGroupSettingExcel>);
		...
		var _ArenaSimulatorFixed = TableEncryptionService.UseEncryption ? TableEncryptionService.Convert(_o.ArenaSimulatorFixed, key) : _o.ArenaSimulatorFixed;
    var _PresetType = default(VectorOffset);
    if (_o.PresetType != null) {
      var __PresetType = new StringOffset[_o.PresetType.Count];
      for (var _j = 0; _j < __PresetType.Length; ++_j) { __PresetType[_j] = builder.CreateString((TableEncryptionService.UseEncryption ? TableEncryptionService.ConvertPack(_o.PresetType[_j], key) : _o.PresetType[_j])); }
